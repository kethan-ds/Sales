...
SILVER_ALL_DATES_TABLE = f"`{CATALOG_NAME}`.`silver`.`all_dates`"
...
# MAGIC %md
# MAGIC # Extract Trades

# COMMAND ----------

#Trade table references (fully-qualified UC three-level names)
# NOTE: "td_cowen_wra" does not appear in create_uc_tables_adf.py's table mapping
# list - confirm the actual UC table name before running.
SILVER_GFI_PATH = f"`{CATALOG_NAME}`.`silver`.`fi_trades`"
...
SILVER_FX_PANO_PATH = f"`{CATALOG_NAME}`.`silver`.`fx_panoramic`"
...
gfi = spark.table(SILVER_GFI_PATH)
...
# #old, new
gfi = (gfi
    .withColumnRenamed("tradeId", "trade_id")
    .withColumnRenamed("sourceSystem", "source_system")
    .withColumnRenamed("tradeDate", "trade_date")
    .withColumnRenamed("tradeDescription", "trade_description")
    .withColumnRenamed("tradeCurrency", "trade_currency")
    .withColumnRenamed("isClientFacing", "is_client_facing")
    .withColumnRenamed("yearMonth", "year_month")
    .withColumnRenamed("fiscalTradeQuarter", "fiscal_trade_quarter")
    .withColumnRenamed("c360TradeExecutionType", "c360_trade_execution_type")
    .withColumnRenamed("c360MultiLegType", "c360_multi_leg_type")
    .withColumnRenamed("c360TradeType", "c360_trade_type")
    .withColumnRenamed("maturityDate", "maturity_date")
    .withColumnRenamed("volumeTradeCurrency", "volume_trade_currency")
    .withColumnRenamed("volumeCadMM", "volume_cad_mm")
    .withColumnRenamed("volumeUsdMM", "volume_usd_mm")
    .withColumnRenamed("volumeTradeCurrencyMM", "volume_trade_currency_mm")
    .withColumnRenamed("c360SettlementAmount", "c360_settlement_amount")
    .withColumnRenamed("securityID", "security_id")
    .withColumnRenamed("securityDescription", "security_description")
    .withColumnRenamed("securityIDType", "security_id_type")
    .withColumnRenamed("riskOn", "risk_on")
    .withColumnRenamed("traderRegion", "trader_region")
    .withColumnRenamed("legacyProductLevel0", "legacy_product_level_0")
    .withColumnRenamed("legacyProductLevel1", "legacy_product_level_1")
    .withColumnRenamed("legacyProductLevel2", "legacy_product_level_2")
    .withColumnRenamed("legacyProductLevel3", "legacy_product_level_3")
    .withColumnRenamed("productLevel0", "product_level_0")
    .withColumnRenamed("productLevel1", "product_level_1")
    .withColumnRenamed("productLevel2", "product_level_2")
    .withColumnRenamed("productLevel3", "product_level_3")
    .withColumnRenamed("counterPartyCode", "counterparty_code")
    .withColumnRenamed("rawClientName", "raw_client_name")
    .withColumnRenamed("salesPersonACF2", "salesperson_acf2")
    .withColumnRenamed("dv01TradeCurrency", "dv01_trade_currency")
    .withColumnRenamed("dv01Cad", "dv01_cad")
    .withColumnRenamed("dv01Usd", "dv01_usd")
    .withColumnRenamed("cadUsdFxRate", "cad_usd_fx_rate")
    .withColumnRenamed("srcCadFxRate", "src_cad_fx_rate")
    .withColumnRenamed("c360CashSpread", "c360_cash_spread")
    .withColumnRenamed("c360YieldSpread", "c360_yield_spread")
    .withColumnRenamed("c360EstimatedSpread", "c360_estimated_spread")
    .withColumnRenamed("c360CashAsk", "c360_cash_ask")
    .withColumnRenamed("c360CashBid", "c360_cash_bid")
    .withColumnRenamed("c360YieldAsk", "c360_yield_ask")
    .withColumnRenamed("c360YieldBid", "c360_yield_bid")
    .withColumnRenamed("c360CashSpreadSource", "c360_cash_spread_source")
    .withColumnRenamed("c360YieldSpreadSource", "c360_yield_spread_source")
    .withColumnRenamed("clientValueCad", "client_value_cad")
    .withColumnRenamed("clientValueUsd", "client_value_usd")
    .withColumnRenamed("clientValueMarkupValueCad", "client_value_markup")
    .withColumnRenamed("preComputedClientValueCad", "precomputed_client_value_cad")
)

# COMMAND ----------

gfi = apply_schema_data_types(gfi, all_dates_silver_schema)

# COMMAND ----------

# MAGIC %md
# MAGIC ### X_Dealer (Without CAD PRIME)
...
# MAGIC %md
# MAGIC ### CAD PRIME Monthly (Up to July 2025)

# COMMAND ----------

cad_prime_monthly = spark.table(SILVER_CAD_PRIME_MONTHLY_PATH)
...
#Keep only trades on July 2025 or before
cad_prime_monthly = cad_prime_monthly.filter(F.col("trade_date") <= F.lit("2025-07-31"))

# COMMAND ----------

# MAGIC %md
# MAGIC ### CAD PRIME Daily (August 2025 Onwards)
...
# MAGIC %md
# MAGIC ### CAD PRIME CV Securities Borrowed Daily
...
cad_prime_cv_sb_daily = (cad_prime_cv_sb_daily
    .withColumnRenamed("client_value", "client_value_cad")
    .withColumnRenamed("update_time", "ingestion_timestamp")
...
fx_pano = spark.table(SILVER_FX_PANO_PATH)

# COMMAND ----------

fx_pano = fx_pano.drop("trade_id")

# COMMAND ----------

# Rename FX Panoramic Columns
# #old, new
fx_pano = (fx_pano
    .withColumnRenamed("original_trade_id", "trade_id")
    .withColumnRenamed("customer_long_name", "raw_client_name")
    .withColumnRenamed("customer_long_name", "client_display_name")
    .withColumnRenamed("murex_mnemonic", "counterparty_code")
    .withColumnRenamed("currency_group", "product_level_1")
    .withColumnRenamed("product_type", "product_level_2")
    .withColumnRenamed("currency_pair", "trade_currency")
    .withColumnRenamed("side", "direction")
...
fx_pano = (fx_pano
    .withColumn("volume_cad_mm", (F.col("volume_cad").cast("decimal(18,6)")/1000000))
    .withColumn("volume_usd_mm", (F.col("volume_usd").cast("decimal(18,6)")/1000000))
    .withColumn("product_level_0", F.lit("Global FX"))
)

# COMMAND ----------

fx_pano = apply_schema_data_types(fx_pano, all_dates_silver_schema)

# COMMAND ----------

client_exclusion_list = [
    'TDSI Cash Equities',
    'TD Bank National Association (ISD)',
    'TDSI ETF',
    'TDSI Accounting', 'TD Internal', 'TDSI Finance',
    'TD Branch', 'TDSI Equity Derivatives', 'TDSI Prime Brokerage',
    'TD Equipment Finance', 'TD Energy Trading', 'TD Dealer Finance',
    'TD Prime Services', 'TD Trust', 'TDSI Fixed Income US',
    'TD Bank National Association', 'TD Bank London',
    'TD Bank Commercial Branch Trades', 'TD Funding', 'TD Bank',
    'TD Trade Finance',
]
client_exclusion_list_lower = [c.lower() for c in client_exclusion_list]
fx_pano = fx_pano.filter(~F.lower(F.col("raw_client_name")).isin(client_exclusion_list_lower))
...
# MAGIC %md
# MAGIC # Combine Trades datasets

# COMMAND ----------

@enhanced_errors()
def concat_dataframes(dataframes):
    return reduce(lambda x, y: x.unionByName(y, allowMissingColumns=True), dataframes)


# COMMAND ----------

trades_list = [gfi, x_dealer, fx_pano, cowen, cad_prime_monthly, cad_prime_daily, cad_prime_cv_sb_daily, us_prime, fund_finance_cfs, fund_finance_act, wra]
...
# MAGIC %md
# MAGIC #Client Value waterfall

# COMMAND ----------

@enhanced_errors()
def apply_client_value_waterfall(df):
    df = df.withColumn("client_value_cad",
        F.when((F.col("product_level_2") == "FI Derivatives") &
               (F.col("book").isin(['GVNF', 'GVFN', 'GVSD', 'GVAD', 'GVCL', 'GVAC', 'GVMK', 'GVAB', 'GVMC'])),
               F.col("precomputed_client_value_cad"))
...
ad = apply_client_value_waterfall(ad)
...
all_dates = (ad.alias("trades")
    .join(src_cad_fx.alias("fx"),
          ...
          "left").select("trades.*", F.coalesce(F.col("trades.src_cad_fx_rate"), F.col("fx.fxRate")).alias("src_cad_fx_rate_new"))
    .drop(F.col("trades.src_cad_fx_rate")).withColumnRenamed("src_cad_fx_rate_new", "src_cad_fx_rate")
)

# COMMAND ----------

#Set src_cad_fx_rate to 1 when trade_currency is CAD
...
all_dates = (all_dates.alias("trades")
    .join(cad_usd_fx.alias("fx"), (F.col("trades.trade_date") == F.col("fx.fxDate")), "left")
...
# MAGIC %md
# MAGIC ### Do Currency Exchange for CV, Volume and DV01
...
def apply_fx_conversions(df):
    df = (df
        .withColumn('volume_trade_currency_mm', F.when((F.col('volume_trade_currency_mm').isNull()) & (F.col('volume_trade_currency').isNotNull()), F.col('volume_trade_currency')/1000000).otherwise(F.col('volume_trade_currency_mm')))
        .withColumn('volume_cad_mm', F.when((F.col('volume_cad_mm').isNull()) & (F.col('src_cad_fx_rate').isNotNull()) & (F.col('volume_trade_currency_mm').isNotNull()), F.col('volume_trade_currency_mm') * F.col('src_cad_fx_rate')).otherwise(F.col('volume_cad_mm')))
        .withColumn('volume_cad_mm', F.when((F.col('volume_cad_mm').isNull()) & (F.col('cad_usd_fx_rate').isNotNull()) & (F.col('volume_usd_mm').isNotNull()), F.col('volume_usd_mm') * (1/F.col('cad_usd_fx_rate'))).otherwise(F.col('volume_cad_mm')))
        .withColumn('volume_usd_mm', F.when((F.col('volume_usd_mm').isNull()) & (F.col('volume_cad_mm').isNotNull()) & (F.col('cad_usd_fx_rate').isNotNull()), F.col('volume_cad_mm') * F.col('cad_usd_fx_rate')).otherwise(F.col('volume_usd_mm')))
        .withColumn('client_value_cad', F.when((F.col('client_value_cad').isNull()) & (F.col('cad_usd_fx_rate').isNotNull()), F.col('client_value_usd') * (1/F.col('cad_usd_fx_rate'))).otherwise(F.col('client_value_cad')))
        .withColumn('client_value_usd', F.when((F.col('client_value_usd').isNull()) & (F.col('cad_usd_fx_rate').isNotNull()), F.col('client_value_cad') * F.col('cad_usd_fx_rate')).otherwise(F.col('client_value_usd')))
        .withColumn('dv01_cad', F.when((F.col('dv01_cad').isNull()) & (F.col('cad_usd_fx_rate').isNotNull()), F.col('dv01_usd') * (1/F.col('cad_usd_fx_rate'))).otherwise(F.col('dv01_cad')))
        .withColumn('dv01_usd', F.when((F.col('dv01_usd').isNull()) & (F.col('cad_usd_fx_rate').isNotNull()), F.col('dv01_cad') * F.col('cad_usd_fx_rate')).otherwise(F.col('dv01_usd')))
    )
    return df
...
pano_source_systems = [row.source_system for row in gfi.select("source_system").union(fx_pano.select("source_system")).na.drop().distinct().collect()]
...
all_dates_refresh_clients = (all_dates_refresh_clients
    .join(ref_data_clients, (F.col("source_system") == F.col("c_source_system")) & (F.col("counterparty_code") == F.col("c_counterparty_code")), "left")
    .join(paam_clients, (F.col("source_system") == F.col("p_source_system")) & (F.col("counterparty_code") == F.col("p_counterparty_code")), "left")
)
...
all_dates_refresh_clients = (all_dates_refresh_clients
    ...
    .withColumn('client_display_name_type', F.when(use_ref_data, F.col('c_client_display_name_type')).when(use_paam_data, F.col('p_client_display_name_type')).otherwise(F.lit(None)))
    .withColumn('client_display_name_top_level', F.when(use_ref_data, F.col('c_client_display_name_top_level')).when(use_paam_data, F.col('p_client_display_name_top_level')).otherwise(F.lit(None)))
    .withColumn('client_display_name_top_level_type', F.when(use_ref_data, F.col('c_client_display_name_top_level_type')).when(use_paam_data, F.col('p_client_display_name_top_level_type')).otherwise(F.lit(None)))
    .withColumn('focused_account', F.when(use_ref_data, F.col('c_focused_account')).otherwise(F.lit(None)))
)
...
def _get_trading_id_lookups():
    ...
    response = _post_ref_data(endpoint=endpoint)
...
@enhanced_errors()
def _repalce_column_with_acf2Id(df, trading_id_lookups, column_to_replace):
    df = (df.alias("trades")
...
def _apply_salesperson_from_trade(df):
    ...
    df = _repalce_column_with_acf2Id(df, trading_id_lookups, "salesperson_acf2")
    df = _repalce_column_with_acf2Id(df, trading_id_lookups, "trader")
    ...
    df = df.withColumn("salesperson_source", F.when(F.col("salesperson_acf2").isin(org_mappings_distinct_acf2Id), F.lit("Ticketed By")).otherwise(F.col("salesperson_source")))
    df = df.withColumn("salesperson_source", F.when((F.col("salesperson_acf2").isin(org_mappings_distinct_acf2Id)) & (F.col("trader").isin(org_mappings_distinct_acf2Id)), F.lit("Trader On Ticket")).otherwise(F.col("salesperson_source")))
...
# MAGIC %md
# MAGIC ##Final Clean up and Standardizations
...
def final_standardizations(df):
    df = df.withColumn('year_month', F.date_format("trade_date", "yyyy_MM"))
    df = df.withColumn("fiscal_trade_quarter", F.concat(
        F.lit("FY"),
        F.when(F.month("trade_date") >= 11, F.year("trade_date")+1).otherwise(F.year("trade_date")), F.lit("Q"),
        F.when(F.month("trade_date").isin(11,12,1), 1).when(F.month("trade_date").isin(2,3,4), 2).when(F.month("trade_date").isin(5,6,7), 3).otherwise(4)
    ))
    df = df.withColumn("days_to_maturity", F.datediff("maturity_date", "trade_date").cast("integer"))
    df = df.withColumn("years_to_maturity", F.when(~F.col("days_to_maturity").isNull(), F.format_string("%.5f", F.col("days_to_maturity")/365.25)).otherwise(F.lit(None)))
    #fiscal_trade_quarter
    df = _standardize_boolean_columns(df, 'is_client_facing', '1', '0', None)
    df = df.withColumn('focused_account', F.when(F.col('focused_account').isNull(), F.lit("Not Provided")).when(F.col('focused_account') == True, F.lit('Yes')).otherwise(F.lit('No')))
