...
dbutils.widgets.text('ADLS_DESTINATION_PATH', 'data/fixed_income/raw/desks/')
...
ADLS_CONTAINER_NAME = os.getenv('ADLS_CONTAINER_NAME', 'tdscicip')
ADLS_ACCOUNT_URI = os.getenv('ADLS_ACCOUNT_URI', 'https://tdscidevadls1.dfs.core.windows.net')

ADB_TENANT_ID = os.getenv('ADB_TENANT_ID', '0ecb99ea-ca1a-4be6-96cc-ceb57b7b63d4')
ADB_SP_APP_ID = os.getenv('ADB_SP_APP_ID', '')
ADB_SP_PWD = os.getenv('ADB_SP_APP_NAME', '')


# COMMAND ----------

# MAGIC %md
# MAGIC # Setup ADLS Client

# COMMAND ----------

# Setup ADLS client
credential = ClientSecretCredential(
    tenant_id=ADB_TENANT_ID,
    client_id=dbutils.secrets.get("tdsci-base", ADB_SP_APP_ID),
    client_secret=dbutils.secrets.get("tdsci-base", ADB_SP_PWD)
)


service_client = DataLakeServiceClient(
    account_url=ADLS_ACCOUNT_URI,
    credential=credential
)

file_system_client = service_client.get_file_system_client(ADLS_CONTAINER_NAME)
directory_client = file_system_client.get_directory_client(directory=ADLS_DESTINATION_PATH)
...
# MAGIC %md
# MAGIC # Upload results to ADLS file path

# COMMAND ----------


file_name = response.headers['Content-Disposition'].split('filename=')[1].split(';')[0]
fc = directory_client.get_file_client(file_name)
