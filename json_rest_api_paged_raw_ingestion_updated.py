...
# MAGIC
# MAGIC REMOVED vs hive_metastore version: create_delta_table() with an explicit ADLS
# MAGIC LOCATION. In UC we create a managed table (no LOCATION) under the catalog so
# MAGIC storage is governed entirely by Unity Catalog.
...
# MAGIC
# MAGIC REMOVED vs hive_metastore version: get_adls_base_path() + spark.read...load(path)
# MAGIC for every silver table. Reading via catalog.schema.table names instead of raw
# MAGIC ADLS paths.
# MAGIC
# MAGIC NOTE: GFI (silver.fi_trades) is OUT OF SCOPE for this release and has been
# MAGIC commented out below. Re-enable when GFI is brought back into scope.

# COMMAND ----------

# silver_gfi_table = f"`{CATALOG_NAME}`.silver.fi_trades"  # GFI out of scope for this release
...
# MAGIC %md
# MAGIC # GFI (OUT OF SCOPE FOR THIS RELEASE - commented out)
...
# MAGIC
# MAGIC NOTE: `gfi` is excluded from trades_list since GFI is out of scope for this
# MAGIC release.
...
trades_list = [
    # gfi,  # GFI out of scope for this release
    x_dealer,
    fx_pano,
    cowen,
    cad_prime_monthly,
...
# MAGIC
# MAGIC REMOVED vs hive_metastore version: DeltaTable.forPath(spark, adls_path).
# MAGIC Using DeltaTable.forName with the catalog-qualified table name instead, since
# MAGIC UC governs the table's storage location - the notebook no longer needs to know
# MAGIC the underlying ADLS path at all.
